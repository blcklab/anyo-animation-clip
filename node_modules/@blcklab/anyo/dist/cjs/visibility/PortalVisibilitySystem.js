"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.PortalVisibilitySystem = void 0;
class PortalVisibilitySystem {
    context;
    options;
    previousRoom;
    visibleRooms = new Set();
    constructor(context, options = {}) {
        this.context = context;
        this.options = options;
    }
    update() {
        if (this.options.enabled === false || this.context.document.visibility?.enabled === false)
            return;
        const currentRoom = this.context.world.getCurrentRoom();
        if (currentRoom === this.previousRoom)
            return;
        this.previousRoom = currentRoom;
        this.recalculate(currentRoom);
    }
    recalculate(currentRoom) {
        const hideUnreachable = this.options.hideUnreachableRooms
            ?? this.context.document.visibility?.hideUnreachableRooms
            ?? true;
        if (!hideUnreachable || currentRoom === null) {
            for (const room of this.context.compiled.rooms)
                this.setVisible(room.roomId, true);
            return;
        }
        const maxDepth = this.options.maxDepth
            ?? this.context.document.visibility?.maxPortalDepth
            ?? 2;
        const reachable = new Set([currentRoom]);
        const queue = [{ roomId: currentRoom, depth: 0 }];
        while (queue.length > 0) {
            const item = queue.shift();
            if (!item || item.depth >= maxDepth)
                continue;
            for (const portal of this.context.compiled.portals) {
                if (!portal.open)
                    continue;
                const next = portal.fromRoom === item.roomId
                    ? portal.toRoom
                    : portal.toRoom === item.roomId
                        ? portal.fromRoom
                        : null;
                if (!next || reachable.has(next))
                    continue;
                reachable.add(next);
                queue.push({ roomId: next, depth: item.depth + 1 });
            }
        }
        for (const room of this.context.compiled.rooms) {
            this.setVisible(room.roomId, reachable.has(room.roomId));
        }
    }
    setVisible(roomId, visible) {
        const currentlyVisible = this.visibleRooms.has(roomId);
        if (visible === currentlyVisible)
            return;
        if (visible)
            this.visibleRooms.add(roomId);
        else
            this.visibleRooms.delete(roomId);
        this.context.world.setRoomVisibility(roomId, visible);
    }
}
exports.PortalVisibilitySystem = PortalVisibilitySystem;
//# sourceMappingURL=PortalVisibilitySystem.js.map