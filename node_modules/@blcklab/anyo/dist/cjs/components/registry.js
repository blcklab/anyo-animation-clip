...
exports.ComponentTypeRegistry = void 0;
exports.createComponentTypeRegistry = createComponentTypeRegistry;
exports.resolveEntityComponents = resolveEntityComponents;
...
class ComponentTypeRegistry {
    ...
    register(registration) {
        ...
        if (!type)
            throw new Error('Component type registrations require a non-empty type.');
        ...
        return () => {
            if (this.registrations.get(type) === normalized)
...
function createComponentTypeRegistry(options = {}) {
    return new ComponentTypeRegistry(options);
}
function componentData(component) {
    const data = {};
    for (const [key, value] of Object.entries(component)) {
        if (key === 'type' || key === 'id' || key === 'enabled' || value === undefined)
            continue;
        data[key] = structuredClone(value);
    }
    return data;
}
function asInteraction(component) {
    ...
    return {
        ...
        params: isRecord(component.params) ? structuredClone(component.params) : isRecord(selectObject?.params) ? structuredClone(selectObject?.params) : undefined,
...
function asTrigger(component) {
    if (!Array.isArray(component.size) || component.size.length !== 3)
        return undefined;
    return {
        size: component.size,
        once: typeof component.once === 'boolean' ? component.once : undefined,
        onEnter: Array.isArray(component.onEnter) ? component.onEnter : undefined,
        onLeave: Array.isArray(component.onLeave) ? component.onLeave : undefined,
    };
}
...
function legacyComponent(type, data) {
    return { type, ...data };
}
function resolveEntityComponents(entity, options) {
    ...
    if (entity.interaction && !explicitTypes.has('anyo.interactable')) {
        all.unshift(legacyComponent('anyo.interactable', structuredClone(entity.interaction)));
    }
    ...
    if (entity.audio && !explicitTypes.has('anyo.audio')) {
        all.unshift(legacyComponent('anyo.audio', structuredClone(entity.audio)));
    }
    if (entity.lod && !explicitTypes.has('anyo.lod')) {
        all.unshift(legacyComponent('anyo.lod', { levels: structuredClone(entity.lod) }));
    }
    if (entity.trigger && !explicitTypes.has('anyo.trigger')) {
        all.unshift(legacyComponent('anyo.trigger', structuredClone(entity.trigger)));
    }
    ...
    for (const [index, component] of all.entries()) {
        ...
        switch (component.type) {
            case 'anyo.interactable':
                interaction = asInteraction(component);
                dynamic = true;
                break;
            case 'anyo.collider':
                collision = typeof component.enabled === 'boolean' ? component.enabled : true;
                dynamic = true;
                break;
            case 'anyo.audio':
                audio = asAudio(component);
                dynamic = true;
                break;
            case 'anyo.lod':
                lod = asLod(component);
                ...
                break;
            case 'anyo.trigger':
                trigger = asTrigger(component);
                dynamic = true;
                break;
            case 'anyo.visibility':
...
function validateSurfaceHost(component, context) {
    ...
    for (const [slotName, slot] of Object.entries(component.slots)) {
        if (!slotName.trim())
            throw componentError('ANYO_SURFACE_HOST_SLOT_NAME_INVALID', context, 'Field: slots');
        if (!isRecord(slot))
            throw componentError('ANYO_SURFACE_HOST_SLOT_INVALID', context, `Field: slots/${slotName}`);
    ...
    const registrations = [
        ...
        { type: 'anyo.mapFeature' },
        {
            ...
            validate(component, context) {
                ...
                if (isRecord(component.distance)) {
                    ...
                    if (distance.min !== undefined && (typeof distance.min !== 'number' || !Number.isFinite(distance.min) || distance.min <= 0))
                        throw componentError('ANYO_AUDIO_DISTANCE_MIN_INVALID', context, 'Field: distance/min');
                    if (distance.max !== undefined && (typeof distance.max !== 'number' || !Number.isFinite(distance.max) || distance.max <= 0))
                        throw componentError('ANYO_AUDIO_DISTANCE_MAX_INVALID', context, 'Field: distance/max');
                    if (distance.rolloff !== undefined && (typeof distance.rolloff !== 'number' || !Number.isFinite(distance.rolloff) || distance.rolloff < 0))
                        throw componentError('ANYO_AUDIO_DISTANCE_ROLLOFF_INVALID', context, 'Field: distance/rolloff');
                }
                if (isRecord(component.cone)) {
                    const cone = component.cone;
                    for (const field of ['innerAngle', 'outerAngle'])
                        if (cone[field] !== undefined && (typeof cone[field] !== 'number' || !Number.isFinite(cone[field]) || Number(cone[field]) < 0 || Number(cone[field]) > 360))
                            throw componentError('ANYO_AUDIO_CONE_ANGLE_INVALID', context, `Field: cone/${field}`);
                    if (cone.outerGain !== undefined && (typeof cone.outerGain !== 'number' || !Number.isFinite(cone.outerGain) || cone.outerGain < 0 || cone.outerGain > 1))
                        throw componentError('ANYO_AUDIO_CONE_GAIN_INVALID', context, 'Field: cone/outerGain');
                }
            },
        },
        {
            ...
            validate(component, context) {
                ...
                if (!levels || levels.length === 0)
                    throw componentError('ANYO_LOD_COMPONENT_LEVELS_REQUIRED', context, 'Field: levels');
        ...
        { type: 'anyo.animation' },
        {
            ...
            validate(component, context) {
                ...
                if (component.mass !== undefined) {
                    ...
                    if (invalidMass)
                        throw componentError('ANYO_RIGID_BODY_MASS_INVALID', context, 'Field: mass');
                }
                for (const field of ['gravityScale', 'linearDamping', 'angularDamping']) {
        ...
        },
        {
            ...
            validate(component, context) {
                ...
                for (const field of ['walkSpeed', 'runSpeed', 'jumpSpeed', 'acceleration', 'airAcceleration', 'deceleration', 'gravity', 'terminalVelocity', 'stepHeight', 'groundSnap', 'rotationSpeed', 'cameraDistance', 'cameraHeight']) {
                    const value = component[field];
                    if (value !== undefined && (typeof value !== 'number' || !Number.isFinite(value) || value < 0)) {
                        throw componentError('ANYO_CHARACTER_VALUE_INVALID', context, `Field: ${field}`);
                    }
                }
                for (const field of ['maxSlopeAngle', 'minSlideAngle']) {
                    const value = component[field];
        ...
        },
        {
            ...
            validate(component, context) {
                ...
                for (const field of ['anchorA', 'anchorB', 'axis']) {
                    const value = component[field];
