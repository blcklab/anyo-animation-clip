...
exports.compileWorldChannels = compileWorldChannels;
exports.resolveChannelMask = resolveChannelMask;
exports.compileCameras = compileCameras;
exports.resolveRenderingConfiguration = resolveRenderingConfiguration;
exports.buildCompilerDependencyGraph = buildCompilerDependencyGraph;
...
function channelBit(value) {
    return typeof value === 'number' ? value : value.bit;
}
function compileChannelMap(value, defaults) {
    const names = {};
    let allMask = 0;
    for (const [name, raw] of Object.entries(value ?? defaults).sort(([a], [b]) => a.localeCompare(b))) {
        ...
        if (Object.values(names).includes(bit))
            throw new Error(`Channel bit ${bit} is assigned more than once.`);
        names[name] = bit;
        allMask |= bit;
    }
    return { names: Object.freeze(names), allMask };
}
function compileWorldChannels(definition = {}) {
    return {
...
function resolveChannelMask(names, map, fallback = map.allMask) {
    if (!names)
        return fallback;
    let mask = 0;
...
function cameraTransform(definition) {
    return {
...
function compileCameras(cameras = {}, activeCamera, channels = compileWorldChannels()) {
    const output = Object.entries(cameras).sort(([a], [b]) => a.localeCompare(b)).map(([id, definition]) => ({
        id,
    ...
    for (const camera of output) {
        if (camera.parentId && !ids.has(camera.parentId))
            throw new Error(`Camera "${camera.id}" references unknown parent camera "${camera.parentId}".`);
    }
    ...
    if (activeCamera && !selected)
        throw new Error(`activeCamera references unknown camera "${activeCamera}".`);
    if (selected && !selected.enabled)
        throw new Error(`activeCamera "${selected.id}" is disabled.`);
    return { cameras: output, activeCameraId: selected?.id };
}
function qualityIndex(value) {
    return QUALITY_ORDER.indexOf(value);
}
function clampQuality(requested, maximum, supported) {
    let result = requested;
    if (maximum && qualityIndex(result) > qualityIndex(maximum))
        result = maximum;
...
function resolveRenderingConfiguration(intent = {}, host = {}, capabilities = {}) {
    const diagnostics = [];
    ...
    if (quality !== requested)
        diagnostics.push({ severity: 'info', code: 'RENDER_QUALITY_CLAMPED', message: `World quality ${requested} was resolved to ${quality}.` });
    const maximumPixelRatio = Math.max(0.5, Math.min(intent.pixelRatio?.maximum ?? 2, host.maximumPixelRatio ?? Number.POSITIVE_INFINITY, capabilities.maximumPixelRatio ?? Number.POSITIVE_INFINITY));
    const antialiasing = intent.antialiasing === 'auto' || !intent.antialiasing
        ? (quality === 'low' ? 'none' : 'fxaa')
        : intent.antialiasing;
    const toneMapping = intent.toneMapping ?? 'aces';
    for (const [feature, expected] of Object.entries(intent.capabilityRequirements ?? {}).sort(([a], [b]) => a.localeCompare(b))) {
        ...
        if (!capabilityMatches(actual, expected))
            diagnostics.push({
                severity: 'error',
...
function versionSatisfies(actual, requested) {
    ...
    if (compareVersion(a, r) < 0)
        return false;
    if (requested.startsWith('^')) {
        if (r[0] > 0)
            return a[0] === r[0];
        if (r[1] > 0)
            return a[0] === 0 && a[1] === r[1];
...
class ExtensionRegistry {
    ...
    register(manifest) {
        if (!manifest.id.trim())
            throw new Error('Extension manifest id must not be empty.');
        if (!manifest.version.trim())
            throw new Error(`Extension "${manifest.id}" must declare a version.`);
        ...
        for (const claim of claims) {
            ...
            if (owner)
                throw new Error(`Extension namespace collision: ${claim} is already owned by "${owner}".`);
        }
        const stored = cloneManifest(manifest);
        this.manifests.set(manifest.id, stored);
        for (const claim of claims)
            this.ownership.set(claim, manifest.id);
        return () => {
            this.manifests.delete(manifest.id);
            for (const claim of claims)
                if (this.ownership.get(claim) === manifest.id)
                    this.ownership.delete(claim);
        };
    }
    ...
    list() { return [...this.manifests.values()].map(cloneManifest); }
    ...
    migrate(document, fromVersion, toVersion) {
        ...
        for (const manifest of [...this.manifests.values()].sort((a, b) => a.id.localeCompare(b.id))) {
            if (manifest.migrate)
                result = manifest.migrate(result, fromVersion, toVersion);
        }
        return result;
    }
    snapshotHooks() {
        return [...this.manifests.values()]
            .filter((manifest) => Boolean(manifest.snapshot))
            .sort((a, b) => a.id.localeCompare(b.id))
            .map((manifest) => manifest.snapshot);
    }
    schemaContributions() {
        return [...this.manifests.values()]
            .filter((manifest) => Boolean(manifest.schema))
            .sort((a, b) => a.id.localeCompare(b.id))
            .map((manifest) => ({ id: manifest.id, root: structuredClone(manifest.schema) }));
    }
    resolve(requirements = {}, schemaVersion) {
        const diagnostics = [];
        for (const [id, requirement] of Object.entries(requirements)) {
            ...
            for (const capability of requirement.capabilities ?? []) {
                if (!(manifest.capabilities ?? []).includes(capability))
                    diagnostics.push({ severity: 'error', code: 'EXTENSION_CAPABILITY_MISSING', extension: id, message: `Extension "${id}" does not provide capability "${capability}".` });
            }
        }
        return diagnostics;
    }
...
function addConsumer(map, key, consumer) {
    if (!key)
        return;
...
function collectBindings(value, entityId, output) {
    if (Array.isArray(value)) {
        for (const child of value)
            collectBindings(child, entityId, output);
        return;
    }
    ...
    if (typeof record.$bind === 'string')
        addConsumer(output, record.$bind, entityId);
    for (const child of Object.values(record))
        collectBindings(child, entityId, output);
}
function walkEntities(entities, visitor) {
    for (const entity of entities) {
        visitor(entity);
        walkEntities(entity.children ?? [], visitor);
    }
}
function buildCompilerDependencyGraph(document) {
    const materialConsumers = new Map();
    const assetConsumers = new Map();
    const prefabInstances = new Map();
    const cameraDependents = new Map();
    ...
    for (const floor of document.building?.floors ?? [])
        for (const room of floor.rooms)
            walkEntities(room.entities ?? [], collectEntity);
    for (const [prefabId, prefab] of Object.entries(document.prefabs ?? {})) {
        if (prefab.extends)
            addConsumer(prefabInstances, prefab.extends, `prefab:${prefabId}`);
    ...
    for (const [id, camera] of Object.entries(document.cameras ?? {})) {
        if (camera.parent)
            addConsumer(cameraDependents, camera.parent, id);
        if (camera.follow?.entity)
            addConsumer(cameraDependents, camera.follow.entity, id);
        if (camera.lookAt && !Array.isArray(camera.lookAt) && 'entity' in camera.lookAt)
            addConsumer(cameraDependents, camera.lookAt.entity, id);
    }
