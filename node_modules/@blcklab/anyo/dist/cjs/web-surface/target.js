...
exports.resolveWebSurfaceTargetEntity = resolveWebSurfaceTargetEntity;
exports.compileWebSurfaceTarget = compileWebSurfaceTarget;
exports.getWebSurfaceHostSlot = getWebSurfaceHostSlot;
exports.resolveWebSurfaceTarget = resolveWebSurfaceTarget;
function isRecord(value) {
    return Boolean(value) && typeof value === 'object' && !Array.isArray(value);
}
function surfaceHostComponent(entity) {
    return entity.components?.find((component) => component.enabled && component.type === 'anyo.surface-host');
}
function slotFromComponent(component, entityId, slotName) {
    ...
    if (typeof mesh !== 'string' || !mesh.trim())
        return null;
    if (!Number.isInteger(materialSlot) || Number(materialSlot) < 0)
        return null;
    if (uvSet !== undefined && (!Number.isInteger(uvSet) || Number(uvSet) < 0))
        return null;
...
function resolveWebSurfaceTargetEntity(reference, entityId, parentId) {
    if (reference === '$self')
        return entityId;
    if (reference === '$parent')
        return parentId ?? null;
    if (reference.startsWith('$self/'))
        return `${entityId}/${reference.slice('$self/'.length)}`;
    if (reference.startsWith('$parent/'))
        return parentId ? `${parentId}/${reference.slice('$parent/'.length)}` : null;
    const normalized = reference.trim();
    return normalized || null;
}
/** Normalize an optional authoring target while preserving the legacy plane size contract. */
function compileWebSurfaceTarget(target, fallbackSize, entityId, parentId) {
    if (!target || target.type === 'plane') {
        const size = target?.size ?? fallbackSize;
        return Object.freeze({ type: 'plane', size: Object.freeze([size[0], size[1]]) });
    }
    if (target.type === 'wall') {
        return Object.freeze({
            type: 'wall',
            room: target.room,
            wall: target.wall,
            offset: target.offset ? Object.freeze([target.offset[0], target.offset[1]]) : undefined,
        });
    }
...
function getWebSurfaceHostSlot(compiled, entityId, slotName) {
 ...
 */
function resolveWebSurfaceTarget(compiled, primitive) {
    const target = primitive.webSurface.target ?? {
        type: 'plane',
        size: [primitive.size?.[0] ?? 2, primitive.size?.[1] ?? 1],
    };
    if (target.type === 'plane')
        return { resolved: true, kind: 'plane', target, primitive };
    if (target.type === 'wall') {
        const room = compiled.roomById.get(target.room);
        if (room)
            return { resolved: true, kind: 'wall', target, room };
        return {
            resolved: false,
            kind: 'wall',
            target,
    ...
    if (target.type === 'mesh')
        return { resolved: true, kind: 'mesh', target, entity };
    const component = surfaceHostComponent(entity);
    if (!component) {
        return {
            resolved: false,
