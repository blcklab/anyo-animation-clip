...
exports.WebSurfaceRuntime = void 0;
...
function animationFrames(preset) {
    switch (preset) {
...
function createSurfaceElement(definition, primitive) {
...
function presentationIdentity(definition) {
    const source = definition.source;
    if (source.type === 'app')
        return `app:${definition.renderMode}:${source.app}`;
    if (source.type === 'url')
        return `url:${definition.renderMode}:${source.url}:${source.title ?? ''}`;
...
class WebSurfaceRuntime {
    ...
    async sync(context) {
        if (!this.root || this.disposed)
            return;
        const available = new Map(context.compiled.primitives
            .filter((primitive) => Boolean(primitive.webSurface))
            .map((primitive) => [primitive.id, primitive]));
        const current = new Map([...available.values()]
            .filter(primitive => this.options.shouldPresent?.(primitive, context) ?? true)
            .map((primitive) => [primitive.id, primitive]));
        for (const id of this.mounted.keys()) {
            if (!current.has(id))
                this.unmount(id, !available.has(id));
        }
        for (const primitive of current.values()) {
            const existing = this.mounted.get(primitive.id);
            if (!existing)
                await this.mount(primitive, context);
            else
                await this.updateMounted(existing, primitive, context);
        }
    }
    update(context) {
        if (!this.root || this.disposed)
        ...
        for (const surface of this.mounted.values()) {
            ...
            const projection = (0, projection_js_1.projectWebSurface)(context.renderer.camera, surface.primitive);
            if (!projection) {
                this.diagnosticOnce(surface, {
                    severity: 'warning',
    ...
    async mount(primitive, context) {
        ...
        if (mounted.presentationReady)
            this.play(mounted, 'mount');
    }
    async updateMounted(surface, primitive, context) {
    ...
    async remountContent(surface, context) {
        this.setActive(surface, false);
        ...
        if (surface.presentationReady)
            this.play(surface, 'mount');
    }
    async mountContent(surface, context) {
        const source = surface.definition.source;
        surface.presentationReady = false;
        if (source.type === 'snapshot' || surface.definition.renderMode === 'snapshot')
            return;
        if (source.type === 'url') {
            if (surface.definition.renderMode === 'external')
                this.mountExternalLink(surface, source);
            else
                this.mountExternalUrl(surface, source);
            return;
        }
        if (surface.definition.renderMode === 'external') {
    ...
    mountExternalLink(surface, source) {
    ...
    mountExternalUrl(surface, source) {
        const policy = this.options.externalUrls;
        const origin = safeOrigin(source.url);
        if (!policy || !origin || !policy.allowedOrigins.includes(origin)) {
            this.diagnosticOnce(surface, { severity: 'warning', code: 'WEB_SURFACE_URL_BLOCKED', message: `External web surface URL is not allowed: ${source.url}` });
    ...
    canProject(surface, context, xrActive) {
        if (!surface.presentationReady || xrActive || !surface.primitive.visible)
            return false;
    ...
    applyLayout(surface, projection) {
    ...
    setActive(surface, active, manageFallback = true) {
        const next = active && surface.presentationReady;
        if (manageFallback)
            this.setFallbackHidden(surface, next);
        ...
        this.applyInteractionState(surface);
        if (!next)
            this.releaseFocus(surface);
        this.invoke(surface, 'setActive', () => surface.app?.setActive?.(next));
        if (next)
            this.invoke(surface, 'resume', () => surface.app?.resume?.());
        else
            this.invoke(surface, 'pause', () => surface.app?.pause?.());
        this.play(surface, next ? 'visible' : 'hidden');
    }
    setFallbackHidden(surface, hidden) {
        if (this.options.hideFallbackWhileActive === false || !surface.renderer.setPrimitiveVisibility)
            return;
        if (surface.fallbackHidden === hidden)
            return;
    ...
    applyInteractionState(surface) {
        surface.element.style.pointerEvents = surface.active && surface.definition.interaction.pointer ? 'auto' : 'none';
    }
    releaseFocus(surface) {
        const activeElement = globalThis.document.activeElement;
        if (!activeElement || typeof surface.element.contains !== 'function' || !surface.element.contains(activeElement))
            return;
        activeElement.blur?.();
    }
    installAnimationEvents(surface) {
    ...
    play(surface, trigger) {
        if (this.reducedMotion || typeof surface.content.animate !== 'function')
            return;
    ...
    unmount(id, restoreFallback = true) {
    ...
    invoke(surface, operation, callback) {
        try {
            callback();
        }
        catch (error) {
    ...
    diagnoseTargetFallback(surface, context) {
        const target = surface.definition.target;
        if (!target || target.type === 'plane')
            return;
        const resolution = (0, target_js_1.resolveWebSurfaceTarget)(context.compiled, surface.primitive);
    ...
    diagnosticOnce(surface, diagnostic) {
        if (surface.diagnostics.has(diagnostic.code))
            return;
...
exports.WebSurfaceRuntime = WebSurfaceRuntime;
//# sourceMappingURL=WebSurfaceRuntime.js.map