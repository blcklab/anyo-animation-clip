...
exports.RuntimeTransformStore = void 0;
...
function sameInput(left, right) {
    return (sameTuple(left.position, right.position) &&
        sameTuple(left.rotation, right.rotation) &&
        sameTuple(left.quaternion, right.quaternion) &&
        sameTuple(left.scale, right.scale));
}
function asComposable(transform) {
    return {
        position: transform.position,
        rotation: transform.rotation,
        quaternion: transform.quaternion ?? (0, quaternion_js_1.quaternionFromEulerXYZ)(transform.rotation),
...
function applyLayer(current, layer) {
    ...
    if (layer.mode === 'override') {
        const quaternion = value.quaternion
            ? (0, quaternion_js_1.normalizeQuaternion)(value.quaternion)
            : value.rotation
                ? (0, quaternion_js_1.quaternionFromEulerXYZ)(value.rotation)
                : current.quaternion;
        return {
            position: value.position ?? current.position,
            rotation: value.rotation ?? (value.quaternion ? (0, quaternion_js_1.eulerXYZFromQuaternion)(quaternion) : current.rotation),
            quaternion,
    ...
    const additiveQuaternion = value.quaternion
        ? (0, quaternion_js_1.normalizeQuaternion)(value.quaternion)
        : (0, quaternion_js_1.quaternionFromEulerXYZ)(value.rotation ?? [0, 0, 0]);
    const quaternion = (0, quaternion_js_1.multiplyQuaternions)(current.quaternion, additiveQuaternion);
    return {
        position: [
...
function sortLayers(layers) {
    return [...layers].sort((left, right) => (left.priority - right.priority || left.source.localeCompare(right.source)));
}
function relativeComposable(parent, world) {
    try {
        const parentTransform = (0, transform_js_1.finalizeTransform)(parent);
        const worldTransform = (0, transform_js_1.finalizeTransform)(world);
        if (!parentTransform.matrix || !worldTransform.matrix)
            return world;
        return asComposable((0, matrix4_js_1.decomposeMatrix4)((0, matrix4_js_1.multiplyMatrix4)((0, matrix4_js_1.invertMatrix4)(parentTransform.matrix), worldTransform.matrix)));
    }
    catch {
        return world;
    }
}
function inferredStaticParent(entity) {
    if (!entity.localTransform?.matrix || !entity.transform.matrix)
        return null;
    try {
        return asComposable((0, matrix4_js_1.decomposeMatrix4)((0, matrix4_js_1.multiplyMatrix4)(entity.transform.matrix, (0, matrix4_js_1.invertMatrix4)(entity.localTransform.matrix))));
    }
    catch {
        return null;
    }
}
function transformPrimitive(entity, finalEntityTransform, primitive) {
    if (primitive.transform === entity.transform || primitive.id === `entity:${entity.id}`)
        return finalEntityTransform;
    ...
    if (entityMatrix && finalMatrix && primitiveMatrix) {
        try {
            const delta = (0, matrix4_js_1.multiplyMatrix4)(finalMatrix, (0, matrix4_js_1.invertMatrix4)(entityMatrix));
            return (0, matrix4_js_1.decomposeMatrix4)((0, matrix4_js_1.multiplyMatrix4)(delta, primitiveMatrix));
        }
        catch {
            // Fall through to a conservative TRS approximation for custom multi-primitive entities.
        }
    }
    const baseQuaternion = entity.transform.quaternion ?? (0, quaternion_js_1.quaternionFromEulerXYZ)(entity.transform.rotation);
    const finalQuaternion = finalEntityTransform.quaternion ?? (0, quaternion_js_1.quaternionFromEulerXYZ)(finalEntityTransform.rotation);
    const primitiveQuaternion = primitive.transform.quaternion ?? (0, quaternion_js_1.quaternionFromEulerXYZ)(primitive.transform.rotation);
    const deltaQuaternion = (0, quaternion_js_1.multiplyQuaternions)(finalQuaternion, [-baseQuaternion[0], -baseQuaternion[1], -baseQuaternion[2], baseQuaternion[3]]);
    const quaternion = (0, quaternion_js_1.multiplyQuaternions)(deltaQuaternion, primitiveQuaternion);
    return (0, transform_js_1.finalizeTransform)({
        position: [
...
class RuntimeTransformStore {
    onDirty;
    compiled = null;
    layers = new Map();
    children = new Map();
    staticParents = new Map();
    dirty = new Set();
    resolved = new Map();
    influenced = new Set();
    constructor(onDirty) {
    ...
    updateWorld(compiled) {
        ...
        const rebaseId = (previousId) => {
            if (compiled.entityById.has(previousId))
                return previousId;
        ...
        for (const [previousId, bucket] of previousLayers) {
            ...
            if (!nextId)
                continue;
            const target = this.layers.get(nextId) ?? new Map();
            for (const [source, layer] of bucket)
                target.set(source, layer);
    ...
    set(entityId, transform, options) {
        const source = options.source.trim();
        if (!source)
            throw new Error('Runtime transform writes require a non-empty source.');
        ...
        if (!Number.isFinite(priority))
            throw new Error('Runtime transform priority must be finite.');
        const mode = options.mode ?? 'override';
        const space = options.space ?? 'world';
        const next = { source, priority, mode, space, transform: cloneInput(transform) };
        const bucket = this.layers.get(resolvedId) ?? new Map();
        const previous = bucket.get(source);
        if (previous && previous.priority === priority && previous.mode === mode && previous.space === space && sameInput(previous.transform, next.transform))
            return;
        bucket.set(source, next);
        this.layers.set(resolvedId, bucket);
        this.markDirty(resolvedId);
    }
    clear(entityId, source) {
        const resolvedId = this.compiled?.entityByAuthoringId.get(entityId)?.id ?? entityId;
    ...
    clearAll() {
        if (this.layers.size === 0 && this.influenced.size === 0)
            return;
    ...
    getResolved(entityId) {
        const resolvedId = this.compiled?.entityByAuthoringId.get(entityId)?.id ?? entityId;
        return this.resolved.get(resolvedId) ?? this.compiled?.entityById.get(resolvedId)?.transform ?? null;
    }
    snapshotLayers() {
        const snapshot = new Map();
        for (const entityId of this.layers.keys())
            snapshot.set(entityId, this.getLayers(entityId));
        return snapshot;
    }
    restoreLayers(snapshot) {
        ...
        for (const [entityId, layers] of snapshot) {
            for (const layer of layers) {
                const bucket = this.layers.get(entityId) ?? new Map();
    ...
    takeUpdates() {
        ...
        if (!compiled || this.dirty.size === 0)
            return [];
        const affected = new Set();
        for (const entityId of this.dirty)
            this.collectDescendants(entityId, affected);
        this.dirty.clear();
        const depthCache = new Map();
        const depthOf = (entityId) => {
        ...
        for (const entityId of ordered) {
            ...
            if (parent && parentInfluenced) {
                ...
                if (entity.localTransform)
                    inherited = (0, transform_js_1.finalizeTransform)((0, transform_js_1.composeTransforms)(asComposable(parentTransform), asComposable(entity.localTransform)));
                else if (parent.transform.matrix && parentTransform.matrix && entity.transform.matrix) {
                    try {
                        const local = (0, matrix4_js_1.decomposeMatrix4)((0, matrix4_js_1.multiplyMatrix4)((0, matrix4_js_1.invertMatrix4)(parent.transform.matrix), entity.transform.matrix));
                        inherited = (0, matrix4_js_1.decomposeMatrix4)((0, matrix4_js_1.multiplyMatrix4)(parentTransform.matrix, local.matrix));
                    }
                    catch {
            ...
            if (ownLayers?.size) {
                ...
                for (const layer of sortLayers(ownLayers.values())) {
                    ...
                    if (currentLocal && parentWorld) {
                        currentWorld = (0, transform_js_1.composeTransforms)(parentWorld, currentLocal);
                        currentLocal = null;
                    }
                ...
                if (currentLocal && parentWorld)
                    currentWorld = (0, transform_js_1.composeTransforms)(parentWorld, currentLocal);
                finalTransform = (0, transform_js_1.finalizeTransform)(currentWorld);
            }
            if (isInfluenced) {
    ...
    restoreUpdates(updates) {
        for (const update of updates)
            this.dirty.add(update.entityId);
        if (updates.length > 0)
            this.onDirty?.();
    }
    markAllActiveDirty() {
        for (const entityId of new Set([...this.layers.keys(), ...this.influenced]))
            this.dirty.add(entityId);
        if (this.dirty.size > 0)
            this.onDirty?.();
    }
    ...
    collectDescendants(entityId, output) {
        if (output.has(entityId))
            return;
        output.add(entityId);
        for (const childId of this.children.get(entityId) ?? [])
            this.collectDescendants(childId, output);
    }
}
exports.RuntimeTransformStore = RuntimeTransformStore;
//# sourceMappingURL=RuntimeTransformStore.js.map