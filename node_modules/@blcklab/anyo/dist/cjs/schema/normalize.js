...
exports.normalizeWorldDocument = normalizeWorldDocument;
const walls_js_1 = require("../math/walls.js");
const validate_js_1 = require("./validate.js");
const bindings_js_1 = require("./bindings.js");
const visualContract_js_1 = require("../core/visualContract.js");
...
function alignedOffset(parentLength, childLength, align) {
    if (align === 'start')
...
function resolveAttachedPosition(room, parent) {
    const attach = room.attachTo;
    if (!attach)
        return room.position ?? [0, 0];
...
function normalizeOpening(room, opening, index) {
    const wallLength = (0, walls_js_1.getWallLength)(room, opening.wall);
...
function solveRooms(floorId, rooms, defaults) {
    ...
    const solve = (id) => {
        ...
        if (cached)
            return cached;
        if (visiting.has(id))
            throw new Error(`Room attachment cycle detected at "${id}".`);
...
function translatedOpeningOffset(opening, fromRoom, toRoom, wall) {
...
function mirrorOpening(opening, fromRoom, toRoom, wall) {
    return {
        ...opening,
        id: opening.id,
        wall,
...
function addImplicitConnections(floors) {
    const rooms = new Map();
    ...
    for (const floor of floors) {
        for (const room of floor.rooms) {
            ...
            if (!childOpening && !parentOpening) {
                const width = Math.min(1.4, (0, walls_js_1.getWallLength)(room, childWall) * 0.5);
                childOpening = {
...
function mergeComponents(template, instance) {
    ...
    for (const component of instance ?? []) {
        ...
        if (index >= 0)
            result[index] = { ...result[index], ...structuredClone(component) };
        else
            result.push(structuredClone(component));
    }
    return result;
}
...
function createAuthoringReference(entity, sourcePath) {
    return {
...
function annotateEntity(entity, sourcePath) {
    return {
...
function mergeEntityTemplate(template, instance) {
    return {
        ...template,
        ...instance,
        id: instance.id,
...
function markTemplateInstance(entity, instance) {
...
function markRepeated(entity, source, index) {
    const authoring = {
...
function expandRepeat(entity) {
    ...
    for (let index = 0; index < repeat.count; index += 1) {
        ...
        let copy = { ...structuredClone(entity), id: `${entity.id}:${index}`, repeat: undefined };
        if (copy.surface) {
...
function applyPrefabOverrides(template, overrides) {
    ...
    for (const [pointer, value] of Object.entries(overrides)) {
        ...
        if (parts.length === 0)
            throw new Error('Prefab override paths must not replace the complete prefab.');
        let target = result;
        for (const part of parts.slice(0, -1)) {
            if (Array.isArray(target)) {
                const index = Number(part);
                if (!Number.isInteger(index) || index < 0 || index >= target.length)
                    throw new Error(`Prefab override path "${pointer}" is outside the template.`);
                target = target[index];
            }
            else if (target && typeof target === 'object' && (0, safePath_js_1.hasOwn)(target, part))
                target = target[part];
            else
                throw new Error(`Prefab override path "${pointer}" does not exist.`);
        }
        ...
        if (Array.isArray(target)) {
            ...
            if (!(0, safePath_js_1.hasOwn)(target, key))
                throw new Error(`Prefab override path "${pointer}" does not exist.`);
            target[key] = structuredClone(value);
        }
        else
            throw new Error(`Prefab override path "${pointer}" cannot be assigned.`);
    }
    return result;
}
function resolvePrefabDefinition(id, prefabs, stack = []) {
    if (stack.includes(id))
        throw new Error(`Prefab inheritance cycle detected: ${[...stack, id].join(' -> ')}`);
...
function namespaceChildIds(entity) {
...
function collectEntityIds(entity, ids) {
    if (ids.has(entity.id))
        throw new Error(`Expanded entity id "${entity.id}" is duplicated.`);
    ids.add(entity.id);
    for (const child of entity.children ?? [])
        collectEntityIds(child, ids);
}
function flattenEntities(document) {
    const raw = (document.entities ?? []).map((entity, index) => annotateEntity(entity, `/entities/${index}`));
    ...
    const visit = (entity) => {
        ...
        if (authoringId) {
            if (authoringIds.has(authoringId))
                throw new Error(`Authoring identity "${authoringId}" is duplicated.`);
...
function resolveUrl(value, sourceContext) {
...
function normalizeAsset(asset, sourceContext) {
...
function normalizeComponentUrls(component, sourceContext) {
    const normalized = structuredClone(component);
    if (typeof normalized.src === 'string')
        normalized.src = resolveUrl(normalized.src, sourceContext);
    if (typeof normalized.source === 'string')
        normalized.source = resolveUrl(normalized.source, sourceContext);
    if (typeof normalized.texture === 'string')
        normalized.texture = resolveUrl(normalized.texture, sourceContext);
    ...
    if (levels) {
        const resolved = levels.map((entry) => {
            ...
            if (typeof record.src === 'string')
                record.src = resolveUrl(record.src, sourceContext);
            return record;
        });
        if (Array.isArray(normalized.levels))
            normalized.levels = resolved;
        else
            normalized.lod = resolved;
    }
    return normalized;
}
function normalizeEntityUrls(entity, sourceContext) {
...
function normalizeWorldDocument(input, options = {}) {
    const document = (0, bindings_js_1.resolveDocumentBindings)(input);
    (0, validate_js_1.validateWorldDocument)(document);
    ...
    const assets = Object.fromEntries(Object.entries(document.assets ?? {}).map(([id, asset]) => [id, normalizeAsset(asset, options.sourceContext)]));
    ...
    return {
        ...
        environment: (0, visualContract_js_1.normalizeEnvironmentDefinition)(document.environment),
        materials: Object.fromEntries(Object.entries(document.materials ?? {}).map(([id, material]) => [id, (0, visualContract_js_1.normalizeMaterialDefinition)(material)])),
        assets,
        entities,
        building: {
            ...building,
