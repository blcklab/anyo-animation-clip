...
class InteractionManager {
    ...
    constructor(context, options = {}) {
        ...
        if (options.hover ?? true)
            this.context.renderer.canvas.addEventListener('pointermove', this.onPointerMove);
    }
    dispose() {
        if (this.disposed)
            return;
    ...
    onClick = (event) => {
        if (this.context.world.xr.state === 'active')
            return;
    ...
    onPointerMove = (event) => {
        if (this.context.world.xr.state === 'active')
            return;
        if (document.pointerLockElement === this.context.renderer.canvas)
            return;
        const primitive = this.pick(event);
        if (primitive === this.hovered)
            return;
        if (this.hovered) {
    ...
    pickResult(event) {
        ...
        const distance = result.distance ?? vec3_js_1.vec3.distance(this.context.renderer.camera.getPosition(), primitive.transform.position);
