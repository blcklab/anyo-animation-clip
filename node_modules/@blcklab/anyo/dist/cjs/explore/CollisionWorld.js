...
function horizontalCircleIntersects(position, radius, collider) {
...
function verticalOverlap(position, shape, collider) {
...
function pointInsideXZ(position, collider, inset = 0) {
    return (position[0] >= collider.bounds.min[0] + inset &&
...
class CollisionWorld {
    ...
    move(position, delta, velocityY, shape) {
        let next = [position[0], position[1], position[2]];
    ...
    canOccupy(position, shape) {
        return this.findBlocking(position, shape) === null;
    }
    findSupportAt(position, maxHeight = Number.POSITIVE_INFINITY) {
        return this.findSupport(position, maxHeight);
    }
    moveAxis(position, amount, axis, shape) {
        ...
        for (let index = 0; index < steps; index += 1) {
            ...
            if (climb > 0 && climb <= shape.stepHeight) {
                const stepped = [candidate[0], current[1] + climb + 0.002, candidate[2]];
    ...
    findBlocking(position, shape) {
        for (const collider of this.colliders) {
            if (!collider.enabled || collider.kind === 'floor')
                continue;
            if (!verticalOverlap(position, shape, collider))
                continue;
            if (horizontalCircleIntersects(position, shape.radius, collider))
                return collider;
        }
        return null;
    }
    findSupport(position, maxHeight) {
        let support = null;
        for (const collider of this.colliders) {
            if (!collider.enabled)
                continue;
            if (collider.kind !== 'floor' && collider.kind !== 'stair' && collider.kind !== 'solid')
                continue;
            if (!pointInsideXZ(position, collider))
                continue;
            const top = collider.bounds.max[1];
            if (top <= maxHeight && (support === null || top > support))
                support = top;
        }
        return support;
    }
...
//# sourceMappingURL=CollisionWorld.js.map