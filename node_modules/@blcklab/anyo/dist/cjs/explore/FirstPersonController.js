...
exports.FirstPersonController = void 0;
const aabb_js_1 = require("../math/aabb.js");
const vec3_js_1 = require("../math/vec3.js");
const CollisionWorld_js_1 = require("./CollisionWorld.js");
...
class FirstPersonController {
    ...
    constructor(context, options = {}) {
        ...
        this.xrCleanups.push(context.world.on('xr:session-start', () => {
            this.suspended = true;
            this.clearInput();
            this.releasePointerLock();
        }), context.world.on('xr:session-end', () => { this.suspended = false; }), context.world.on('world:stop', () => {
            this.clearInput();
            this.releasePointerLock();
        }));
        if (options.browserInput !== false)
            this.installBrowserInput();
    }
    get enabled() { return this.enabledState; }
    get inputEnabled() { return this.inputEnabledState; }
    setEnabled(enabled) {
    ...
    setMoveAxes(right, forward) {
        this.externalMove = [(0, vec3_js_1.clamp)(Number.isFinite(right) ? right : 0, -1, 1), (0, vec3_js_1.clamp)(Number.isFinite(forward) ? forward : 0, -1, 1)];
    }
    setRun(running) { this.externalRun = Boolean(running); }
    ...
    update(deltaSeconds) {
        if (this.disposed || !this.enabledState || !this.inputEnabledState || this.suspended || this.context.world.xr.state === 'active')
            return;
        ...
        const moveForward = (0, vec3_js_1.clamp)(keyboardForward + touchAxes[1] + this.externalMove[1], -1, 1);
        const moveRight = (0, vec3_js_1.clamp)(keyboardRight + touchAxes[0] + this.externalMove[0], -1, 1);
        ...
        const result = this.collisions.move(camera.getPosition(), [horizontal[0], vertical, horizontal[2]], this.velocityY, {
    ...
    dispose() {
        ...
        for (const cleanup of this.xrCleanups.splice(0))
            cleanup();
    }
    canAcceptInput() {
    ...
    installBrowserInput() {
        if (this.browserInputInstalled || typeof window === 'undefined' || typeof document === 'undefined')
            return;
    ...
    uninstallBrowserInput() {
        if (!this.browserInputInstalled || typeof document === 'undefined')
            return;
    ...
    onCanvasClick = () => {
        ...
        if (!this.config.pointerLock || document.pointerLockElement === this.canvas)
            return;
    ...
    applyLook(deltaX, deltaY, sensitivity) {
        ...
        this.pitch = (0, vec3_js_1.clamp)(this.pitch - deltaY * sensitivity, -Math.PI / 2 + 0.02, Math.PI / 2 - 0.02);
    ...
    onTouchMove = (event) => {
        ...
        event.preventDefault();
        for (const touch of Array.from(event.changedTouches)) {
            if (touch.identifier === this.touch.moveId)
                this.touch.moveCurrent = [touch.clientX, touch.clientY];
    ...
    getTouchMoveAxes() {
        if (this.touch.moveId === null)
            return [0, 0];
        const dx = (0, vec3_js_1.clamp)((this.touch.moveCurrent[0] - this.touch.moveStart[0]) / 50, -1, 1);
        const dy = (0, vec3_js_1.clamp)((this.touch.moveCurrent[1] - this.touch.moveStart[1]) / 50, -1, 1);
        return [dx, -dy];
    }
    detectRoom(position) {
