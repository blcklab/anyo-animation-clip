export class AssetTypeRegistry {
    ...
    constructor(options = {}) {
        if (options.builtins ?? true)
            registerBuiltInAssetTypes(this);
    }
    register(registration) {
        const type = registration.type.trim();
        if (!type)
            throw new Error('Asset type registrations require a non-empty type.');
    ...
    get(type) { return this.registrations.get(type); }
    has(type) { return this.registrations.has(type); }
    list() { return [...this.registrations.values()]; }
}
export function createAssetTypeRegistry(options = {}) {
    return new AssetTypeRegistry(options);
}
export function validateAssetEcosystem(document, options = {}) {
...
function validateMaterialAssets(id, material, document) {
    for (const field of TEXTURE_FIELDS) {
        ...
        if (!asset)
            throw new Error(`ANYO_MATERIAL_ASSET_NOT_FOUND\nMaterial: ${id}\nField: ${String(field)}\nAsset: ${assetId}`);
...
export function registerBuiltInAssetTypes(registry) {
    const builtins = [
