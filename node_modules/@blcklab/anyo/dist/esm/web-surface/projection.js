...
function transformPoint(matrix, point) {
    if (!matrix)
        return point.every(finite) ? point : null;
    const [x, y, z] = point;
    const w = (matrix[3] ?? 0) * x + (matrix[7] ?? 0) * y + (matrix[11] ?? 0) * z + (matrix[15] ?? 1);
    if (!finite(w) || Math.abs(w) <= Number.EPSILON)
        return null;
    const invW = 1 / w;
    const transformed = [
...
export function projectWebSurface(camera, primitive, minimumPixels = 2) {
    if (!camera.projectWorldPoint || !primitive.size)
        return null;
    const width = primitive.size[0] ?? 1;
    const height = primitive.size[1] ?? 1;
    if (!finite(width) || !finite(height) || width <= 0 || height <= 0 || !finite(minimumPixels) || minimumPixels < 0)
        return null;
    ...
    let projected;
    try {
        projected = points.map(point => camera.projectWorldPoint?.(point));
    }
    catch {
        return null;
    }
    if (projected.length !== 5 || projected.some(value => !value || !validProjection(value)))
        return null;
    ...
    if (![projectedWidth, projectedHeight, angle].every(finite))
        return null;
    return {
        centerX: center.x,
        centerY: center.y,
...
function normalizeReadableAngle(value) {
    let angle = value;
