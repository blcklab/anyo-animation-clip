...
export function createWorldSchema(options = {}) {
    ...
    for (const extension of [...(options.extensions ?? [])].sort((a, b) => a.id.localeCompare(b.id))) {
        if (!/^[A-Za-z0-9@][A-Za-z0-9@._/-]*$/.test(extension.id))
            throw new Error(`Invalid extension schema id "${extension.id}".`);
        if (extensionIds.has(extension.id))
            throw new Error(`Extension schema "${extension.id}" is duplicated.`);
        ...
        for (const [name, definition] of Object.entries(extension.definitions ?? {}).sort(([a], [b]) => a.localeCompare(b))) {
            ...
            if (key in defs)
                throw new Error(`Composed schema definition "${key}" is duplicated.`);
        ...
        if (extension.root) {
            const extensionRoot = (properties.extensions.properties ??= {});
            extensionRoot[extension.id] = clone(extension.root);
        }
    }
    if (componentVariants.length) {
        const component = defs.component;
