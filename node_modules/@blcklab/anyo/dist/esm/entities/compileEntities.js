import { resolveEntityComponents } from '../components/index.js';
...
function locateRoom(document, roomId) {
    if (!roomId)
        return null;
...
function resolveEntityTransforms(entity, document, parent) {
    ...
    const local = entity.surface
        ? (() => {
            ...
            return createTransform(surface.position, surface.rotation, multiplyVec3(surface.scale, ownScale));
        })()
        : createTransform(entity.position ?? [0, 0, 0], entity.rotation ?? [0, 0, 0], ownScale);
    if (parent)
        return { local, world: composeTransforms(parent, local) };
    if (entity.surface)
...
function primitiveKind(entity) {
    if (entity.type === 'group' || entity.type === 'trigger')
        return null;
    ...
    if (entity.type === 'box' || entity.type === 'plane' || entity.type === 'cylinder' ||
        entity.type === 'text' || entity.type === 'image' || entity.type === 'model' ||
        entity.type === 'light' || entity.type === 'audio')
        return entity.type;
    return null;
}
function resolvedString(value) { return typeof value === 'string' ? value : undefined; }
function resolvedNumber(value) { return typeof value === 'number' ? value : undefined; }
function resolvedBoolean(value, fallback) { return typeof value === 'boolean' ? value : fallback; }
function resolveSource(entity, document) {
    if (entity.type === 'web-surface' && entity.webSurface) {
        if (entity.webSurface.source.type === 'snapshot')
            return entity.webSurface.source.image;
...
function addToRoomChunk(output, roomId, primitiveId, colliderId) {
    if (!roomId)
        return;
    ...
    if (!chunk)
        return;
    if (primitiveId)
        chunk.primitiveIds.push(primitiveId);
    if (colliderId)
        chunk.colliderIds.push(colliderId);
}
function unsupportedEntityError(entity, sourcePath) {
    return new Error([
...
function wallTargetSurface(entity) {
    const target = entity.type === 'web-surface' ? entity.webSurface?.target : undefined;
    if (!target || target.type !== 'wall' || !CARDINAL_WALLS.has(target.wall))
...
function compileEntity(entity, document, output, parent, options, inheritedRoom, sourcePath = '/entities', parentId, inheritedVisible = true) {
    const target = entity.type === 'web-surface' ? entity.webSurface?.target : undefined;
    ...
    if (!kind) {
        ...
        if (!registration)
            throw unsupportedEntityError(entity, sourcePath);
...
export function compileEntities(document, output, options = {}) {
    output.entities ??= [];
    ...
    for (const primitive of output.primitives) {
        if (!primitive.webSurface)
            continue;
        const resolution = resolveWebSurfaceTarget({ entityById, roomById }, primitive);
        if (!resolution.resolved) {
            options.warn?.([resolution.code, `Entity: ${primitive.entityId ?? primitive.id}`, resolution.message].join('\n'));
        }
    }
}
//# sourceMappingURL=compileEntities.js.map