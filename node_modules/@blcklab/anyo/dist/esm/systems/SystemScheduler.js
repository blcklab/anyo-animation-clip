...
export class SystemScheduler {
    ...
    frameNumber = 0;
    fixedStepNumber = 0;
    droppedTimeWarningEmitted = false;
    active = false;
    constructor(systems, options = {}, transforms, warn) {
        ...
        for (const system of systems) {
            ...
            if (!name)
                throw new Error('World systems require a non-empty name.');
            if (names.has(name))
                throw new Error(`Duplicate Anyo world system name "${name}".`);
            names.add(name);
        }
    ...
    beginFrame(deltaSeconds, time, driver) {
        if (!this.active || !this.baseContext || this.systems.length === 0)
            return { fixedSteps: 0, interpolationAlpha: 0 };
    ...
    async applyChanges(changes) {
        ...
        for (const system of this.systems) {
            const context = this.contexts.get(system.name);
            if (context)
                await system.applyChanges?.(changes, context);
        }
    }
    dispose(final = false) {
        if (!this.active && this.contexts.size === 0)
            return;
    ...
    createContext(source) {
        ...
        if (!base)
            throw new Error('Cannot create a system context before the world runtime is ready.');
        return {
            ...base,
            source,
...
//# sourceMappingURL=SystemScheduler.js.map