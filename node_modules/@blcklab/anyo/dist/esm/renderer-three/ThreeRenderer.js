...
export class ThreeRenderer {
    ...
    constructor(options) {
        ...
        this.perspectiveCamera = new THREE.PerspectiveCamera(options.fieldOfView ?? 70, 1, options.near ?? 0.05, options.far ?? 500);
    ...
    async addPrimitive(primitive) {
        ...
        object.traverse?.((child) => {
            ...
            if ('castShadow' in child)
                child.castShadow = this.options.shadows ?? false;
            if ('receiveShadow' in child)
                child.receiveShadow = primitive.tags?.includes('floor') ?? false;
        });
    ...
    async loadModel(primitive) {
        ...
        try {
            ...
            if (!scene)
                throw new Error('The glTF file has no scene.');
            scene.userData.anyoOwnedMaterial = true;
            scene.traverse?.((child) => { child.userData.anyoOwnedMaterial = true; });
            return scene;
        }
        catch (error) {
    ...
    createFallbackModel(primitive) {
        const size = primitive.size ?? [1, 1, 1];
        const mesh = new THREE.Mesh(new THREE.BoxGeometry(size[0], size[1], size[2]), new THREE.MeshStandardMaterial({ color: primitive.color ?? '#a1a1aa', roughness: 0.75 }));
        mesh.userData.anyoOwnedMaterial = true;
        return mesh;
    }
    ...
    createMaterialCacheKey(name, definition) {
    ...
    createStandardMaterial(definition) {
    ...
    disposeObject(object, disposeOwnedMaterials) {
        object.traverse?.((child) => {
            child.geometry?.dispose?.();
            if (!disposeOwnedMaterials && !child.userData?.anyoOwnedMaterial)
                return;
            const materials = Array.isArray(child.material) ? child.material : child.material ? [child.material] : [];
            for (const material of materials) {
                if (this.materialCacheHas(material))
                    continue;
                this.disposeMaterial(material);
            }
        });
    }
    materialCacheHas(material) {
    ...
    assertNotDisposed() {
        if (this.disposed)
            throw new Error('This ThreeRenderer has already been disposed.');
    }
