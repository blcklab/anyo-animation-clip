import { AmbientLight, BoxGeometry, Color, CylinderGeometry, DirectionalLight, Engine, Euler, ImageMesh, InstancedMesh, Matrix4, Mesh, Node, Camera, OrthographicCamera, PerspectiveCamera, PlaneGeometry, PointLight, Scene, StandardMaterial, TextMesh, SEKAI64_VERSION, Vector3, createEngine, } from '@blcklab/sekai64';
...
function nativeLayerMask(primitive) {
...
function inferAssetFormat(src) {
    const lower = src.toLowerCase();
    if (lower.startsWith('data:model/gltf+json'))
        return 'gltf';
    if (lower.startsWith('data:model/vrm'))
        return 'vrm';
    if (lower.startsWith('data:model/gltf-binary') || lower.startsWith('data:application/octet-stream'))
        return 'glb';
    try {
...
function defaultMaterial(primitive) {
    if (primitive.tags?.includes('floor'))
        return { color: '#d7d2c8', roughness: 0.78 };
    if (primitive.tags?.includes('ceiling'))
        return { color: '#fafafa', roughness: 0.95 };
    if (primitive.tags?.includes('wall'))
        return { color: '#f1f0ec', roughness: 0.9 };
    if (primitive.tags?.includes('window'))
        return { color: '#b9dce8', roughness: 0.15, opacity: 0.35, transparent: true, side: 'double' };
    if (primitive.tags?.includes('stair'))
        return { color: '#b9b2a6', roughness: 0.75 };
    return { color: primitive.color ?? '#a8a29e', roughness: 0.7 };
}
function materialKey(name, definition) {
...
function capabilitiesFromEngine(engine, assetLoaders) {
    const features = engine.capabilities.features;
    const materialTextureChannels = [];
    if (features.baseColorTextures)
        materialTextureChannels.push('baseColor');
    if (features.normalTextures)
        materialTextureChannels.push('normal');
    if (features.metallicRoughnessTextures)
        materialTextureChannels.push('roughness', 'metalness', 'metallicRoughness');
    if (features.emissiveTextures)
        materialTextureChannels.push('emissive');
    if (features.occlusionTextures)
        materialTextureChannels.push('occlusion');
    return {
        assetFormats: {
        ...
        materialFeatures: ['normalScale', 'occlusionStrength', 'emissiveIntensity', 'transmission', 'ior', 'thickness', 'attenuation', 'toonShading', 'mtoonShading'],
...
export class Sekai64Renderer {
    ...
    scope = null;
    gltfLoader = null;
    assetLoaders = new AssetLoaderRegistry();
    unitBox = null;
    unitBeveledBox = null;
    unitPlane = null;
    unitCylinder = null;
    ...
    roomGroups = new Map();
    materials = new Map();
    instanceBindings = new Map();
    instanceIds = new Map();
    interactivePrimitiveIds = new Set();
    primitiveByNode = new WeakMap();
    externalEntityByPrimitiveId = new Map();
    externalNodeByPrimitiveId = new Map();
    primitiveCache = new Map();
    diagnostics = [];
    diagnosticListeners = new Set();
    assetProgressListeners = new Set();
    ...
    document = null;
    compiled = null;
    namedCameras = new Map();
    worldChannels = null;
    disposed = false;
    disposal = null;
    xrFrameState = null;
    engineDiagnosticCleanup = null;
    capabilityState = {
        text: false,
        images: false,
        models: false,
    ...
    constructor(options) {
        ...
        this.camera = new Sekai64CameraAdapter(() => this.activeCamera, projection => this.setCameraProjection(projection), this.canvas);
        ...
        this.assetLoaders.register({
            type: 'model',
            formats: ['gltf', 'glb'],
            load: async (request) => {
                if (!this.gltfLoader)
                    throw new Error('Sekai64 glTF loader is not initialized.');
        ...
        for (const loader of options.assetLoaders ?? [])
            this.assetLoaders.register(loader);
    }
    get info() {
    ...
    async mount(compiled, document) {
        ...
        for (const camera of compiled.cameras ?? [])
            this.namedCameras.set(camera.id, camera);
        ...
        const scope = engine.resources.createScope('anyo-compiled-world');
        this.scope = scope;
        ...
        if (compiled.activeCameraId) {
            ...
            if (camera)
                await this.activateCamera(compiled.activeCameraId, camera);
        }
    ...
    async applyChanges(changes, compiled, document) {
        const started = now();
        this.assertAlive();
        this.document = document;
        this.compiled = compiled;
        const requiresBatchRebuild = changes.some((change) => {
            if (change.type === 'primitive-remove')
                return this.instanceBindings.has(change.primitiveId);
        ...
        for (const change of changes) {
            switch (change.type) {
                ...
                case 'camera-activate': {
                    ...
                    if (camera)
                        await this.activateCamera(change.cameraId, camera, change.transition);
                    break;
                }
                ...
                case 'rendering-intent':
                    this.applyVisualConfiguration(this.engine, document);
                    break;
                case 'world-rebuild':
                    await this.mount(compiled, document);
                    break;
    ...
    applyRuntimeTransforms(updates) {
        ...
        for (const update of updates)
            this.applyPrimitiveTransform(update.primitive);
    }
    async updatePrimitive(primitive) {
        ...
        if (previous.material !== primitive.material)
            await this.updatePrimitiveMaterial(primitive);
    ...
    async replacePrimitive(primitive) {
        if (this.instanceBindings.has(primitive.id)) {
            if (this.compiled && this.document)
                await this.mount(this.compiled, this.document);
            return;
        }
    ...
    async updatePrimitiveMaterial(primitive) {
    ...
    async updatePrimitiveContent(primitive) {
        const node = this.nodes.get(primitive.id);
        ...
        if (node instanceof ImageMesh && primitive.kind === 'image') {
            if (primitive.src) {
                ...
                this.trackAsset(primitive.id, () => node.setSource(primitive.src, { signal: controller.signal }).then(() => undefined), this.mountGeneration);
            }
            this.applyTransform(node, primitive);
            this.primitiveCache.set(primitive.id, primitive);
            return;
    ...
    async removePrimitive(primitiveId) {
    ...
    setPrimitiveVisibility(primitiveId, visible) {
        const binding = this.instanceBindings.get(primitiveId);
        if (binding) {
        ...
        if (primitive)
            this.primitiveCache.set(primitiveId, { ...primitive, visible });
    }
    setRoomVisibility(roomId, visible) {
        this.roomGroups.get(roomId)?.setVisible(visible);
    }
    setPortalState(_portalId, _open) {
    ...
    async updateCamera(camera) {
        ...
        if (this.compiled?.activeCameraId === camera.id)
            await this.activateCamera(camera.id, camera);
    }
    async removeCamera(cameraId) {
        this.namedCameras.delete(cameraId);
    }
    async activateCamera(cameraId, camera, _transition) {
        ...
        if (this.compiled)
            this.compiled.activeCameraId = cameraId;
    }
    ...
    setCameraProjection(projection) {
        ...
        const rotation = [current.rotation.x, current.rotation.y, current.rotation.z];
        if (projection.type === 'orthographic') {
            this.orthographicVerticalSize = Math.max(.1, projection.verticalSize);
            if (projection.near !== undefined)
                this.orthographicCamera.near = Math.max(.001, projection.near);
            if (projection.far !== undefined)
                this.orthographicCamera.far = Math.max(this.orthographicCamera.near + .001, projection.far);
            this.activeCamera = this.orthographicCamera;
            this.updateOrthographicViewport();
        }
        else {
            if (projection.fieldOfView !== undefined)
                this.perspectiveCamera.fieldOfView = projection.fieldOfView;
            if (projection.near !== undefined)
                this.perspectiveCamera.near = Math.max(.001, projection.near);
            if (projection.far !== undefined)
                this.perspectiveCamera.far = Math.max(this.perspectiveCamera.near + .001, projection.far);
            this.activeCamera = this.perspectiveCamera;
    ...
    render() {
        ...
        if (!this.engine || !this.scene)
            return;
        if (this.xrFrameState && this.xr.state === 'active')
            this.xr.render(this.scene, this.xrFrameState);
        else
            this.engine.render(this.scene, this.activeCamera);
    ...
    pick(clientX, clientY) {
        this.assertAlive();
        if (!this.scene || !this.capabilityState.picking)
            return null;
        ...
        for (const hit of hits) {
            ...
            if (!primitiveId || !this.interactivePrimitiveIds.has(primitiveId))
                continue;
            const primitive = this.primitiveCache.get(primitiveId);
            return {
                primitiveId,
                ...
                ...(hit.uv ? { uv: [hit.uv.x, hit.uv.y] } : {}),
                ...(hit.uv1 ? { uv1: [hit.uv1.x, hit.uv1.y] } : {}),
    ...
    pickRay(origin, direction, options = {}) {
        this.assertAlive();
        if (!this.scene || !this.capabilityState.picking)
            return null;
     ...
     */
    getNativeAccess() {
        if (!this.engine || !this.scene || this.disposed)
            return null;
        ...
        return Object.freeze({
            ...
            getPrimitiveNode: (primitiveId) => this.nodes.get(primitiveId),
            getRoomNode: (roomId) => this.roomGroups.get(roomId),
            registerExternalNode: (registration) => this.registerExternalNode(registration),
        });
    }
    registerExternalNode(registration) {
        this.assertAlive();
        const { primitiveId, entityId, node, interactive = true } = registration;
        if (!primitiveId.trim())
            throw new Error('External Sekai64 nodes require a non-empty primitiveId.');
        ...
        return () => {
            if (!active)
                return;
            active = false;
            if (this.externalNodeByPrimitiveId.get(primitiveId) !== node)
                return;
            ...
            this.interactivePrimitiveIds.delete(primitiveId);
            if (!node.disposed)
                node.layerMask = previousLayerMask;
        };
    }
    getMetrics() {
        if (this.engine) {
    ...
    disposeAsync() {
        ...
        this.disposal = (async () => {
            ...
            if (typeof asyncEngine.disposeAsync === 'function')
                await asyncEngine.disposeAsync();
            else
                engine.dispose();
        })();
        return this.disposal;
    ...
    async ensureEngine() {
        ...
        this.engineDiagnosticCleanup = engine.on('diagnostic', (diagnostic) => {
            this.reportDiagnostic({ ...diagnostic });
        });
        return engine;
    }
    mountPrimitiveBatches(primitives, generation) {
        ...
        for (const group of batchGroups.values()) {
            if (group.length < 2)
                ...
                this.addInstancedBatch(group);
        }
        for (const primitive of singles)
            void this.addPrimitive(primitive, generation);
    }
    addInstancedBatch(primitives) {
        ...
        primitives.forEach((primitive, index) => {
            ...
            if (primitive.interaction)
                this.interactivePrimitiveIds.add(primitive.id);
    ...
    async addPrimitive(primitive, generation) {
        if (!this.scene || generation !== this.mountGeneration)
            return;
        ...
        if (primitive.kind === 'image' && node instanceof ImageMesh && primitive.src) {
            const controller = this.createAssetController(primitive.id);
            this.trackAsset(primitive.id, () => node.setSource(primitive.src, { signal: controller.signal }).then(() => undefined), generation);
        }
    }
    createNode(primitive) {
    ...
    createAssetController(primitiveId) {
        this.assetControllers.get(primitiveId)?.abort();
        const controller = new AbortController();
        if (this.worldAbort?.signal.aborted)
            controller.abort();
        else
            this.worldAbort?.signal.addEventListener('abort', () => controller.abort(), { once: true });
        this.assetControllers.set(primitiveId, controller);
        return controller;
    }
    trackAsset(primitiveId, run, generation) {
        let resolveQueued = () => undefined;
    ...
    geometryFor(primitive) {
        if (primitive.kind === 'box' && primitive.tags?.includes('entity') && this.unitBeveledBox)
            return this.unitBeveledBox;
        if (primitive.kind === 'box' && this.unitBox)
            return this.unitBox;
        if (primitive.kind === 'plane' && this.unitPlane)
            return this.unitPlane;
        if (primitive.kind === 'cylinder' && this.unitCylinder)
    ...
    materialFor(primitive) {
        ...
        const inferredRole = definition.role ?? (primitive.tags?.some(tag => tag === 'avatar' || tag === 'character' || tag === 'vrm') ? 'character' :
        ...
        if (existing)
            return existing;
        if (!this.scope)
            throw new Error('Sekai64 material scope is not initialized.');
    ...
    queueEnvironmentMap(renderer, assetId, intensity, rotation) {
        ...
        this.trackAsset(key, async () => {
            if (typeof fetch !== 'function' || typeof createImageBitmap !== 'function')
                throw new Error('This runtime cannot decode browser environment images.');
            const response = await fetch(source, { signal: controller.signal });
            if (!response.ok)
                throw new Error(`Environment image request failed with ${response.status}.`);
            ...
            try {
                ...
                if (!context)
                    throw new Error('A 2D canvas context is required to decode the environment map.');
                context.drawImage(bitmap, 0, 0);
                const pixels = context.getImageData(0, 0, bitmap.width, bitmap.height).data;
                if (generation === this.mountGeneration && !controller.signal.aborted)
                    renderer.setEnvironmentMap?.({ width: bitmap.width, height: bitmap.height, pixels, intensity, rotation, label: assetId });
            }
            finally {
                bitmap.close();
            }
    ...
    queueColorLut(renderer, assetId, intensity) {
        const source = this.materialTextureSource(assetId);
        if (!source) {
        ...
        this.trackAsset(key, async () => {
            if (typeof fetch !== 'function')
                throw new Error('This runtime cannot fetch color LUT assets.');
            const response = await fetch(source, { signal: controller.signal });
            if (!response.ok)
                throw new Error(`Color LUT request failed with ${response.status}.`);
            const parsed = parseCubeLut(await response.text(), assetId);
            if (generation === this.mountGeneration && !controller.signal.aborted)
                renderer.setColorGrading?.({ enabled: true, lut: parsed.lut, lutIntensity: intensity });
        }, generation);
    }
    materialTextureSource(assetId) {
        if (!assetId)
            return undefined;
        const asset = this.document?.assets[assetId];
        return typeof asset?.src === 'string' ? asset.src : undefined;
    }
    createLight(primitive) {
        const options = { id: primitive.id, color: primitive.color ?? '#ffffff', intensity: primitive.intensity ?? 1 };
        if (primitive.lightType === 'ambient')
    ...
    applyVisualConfiguration(engine, document) {
        ...
        if (environment.lighting.environmentMap) {
            ...
            renderer.setEnvironmentMap?.(undefined);
        if (colorLutAsset && !this.options.colorGrading?.lut)
            this.queueColorLut(renderer, colorLutAsset, lutIntensity);
    ...
    instanceMatrix(primitive, visible) {
        const scale = visible ? this.effectiveScale(primitive) : [0, 0, 0];
        return new Matrix4().compose(new Vector3(...primitive.transform.position), new Euler(...primitive.transform.rotation), new Vector3(...scale));
    }
    parentFor(primitive) {
        if (!this.scene)
            throw new Error('Sekai64 scene is not mounted.');
        return primitive.roomId ? this.roomGroups.get(primitive.roomId) ?? this.scene : this.scene;
    }
    findPrimitiveId(node) {
        let current = node;
        while (current) {
    ...
    clearMountedWorld() {
        ...
        for (const controller of this.assetControllers.values())
            controller.abort();
        this.assetControllers.clear();
        this.pendingAssets.clear();
        for (const queued of this.assetQueue.splice(0))
            queued.resolve();
        if (this.scene) {
    ...
    emitAssetProgress() {
        ...
        for (const listener of [...this.assetProgressListeners])
            listener(progress);
    }
    ...
    assertAlive() {
        if (this.disposed)
            throw new Error('This Sekai64Renderer has already been disposed.');
    }
}
//# sourceMappingURL=Sekai64Renderer.js.map