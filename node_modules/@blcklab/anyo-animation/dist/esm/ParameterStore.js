export class AnimationParameterStore {
    ...
    reconfigure(definitions) {
        ...
        for (const name of previous.activeTriggers) {
            if (definitions[name]?.type === 'trigger')
    ...
    get(name) {
        ...
        if (definition.type === 'trigger')
            return this.activeTriggers.has(name);
    ...
    setNumber(name, value) {
        ...
        if (!Number.isFinite(value))
            throw new Error(`Animation parameter "${name}" must be a finite number.`);
    ...
    resetTrigger(name) {
        this.assertType(name, 'trigger');
        if (!this.activeTriggers.delete(name))
            return false;
    ...
    setParameter(name, value) {
        ...
        if (definition.type === 'trigger') {
            if (typeof value !== 'boolean')
                throw new Error(`Animation trigger "${name}" expects a boolean.`);
            return value ? this.setTrigger(name) : this.resetTrigger(name);
        }
        if (definition.type === 'boolean') {
            if (typeof value !== 'boolean')
                throw new Error(`Animation parameter "${name}" expects a boolean.`);
            return this.setBoolean(name, value);
        }
        if (typeof value !== 'number')
            throw new Error(`Animation parameter "${name}" expects a number.`);
    ...
    bind(parameter, binding) {
        ...
        if (definition.type === 'trigger')
            throw new Error('Trigger parameters cannot use continuous bindings.');
    ...
    restore(snapshot) {
        ...
        for (const name of snapshot.activeTriggers) {
            if (this.definitions[name]?.type === 'trigger')
    ...
    initialize(definitions) {
        for (const [name, definition] of Object.entries(definitions)) {
            if (definition.type === 'boolean')
                this.values.set(name, definition.default ?? false);
            else if (definition.type === 'number')
                this.values.set(name, clampDefault(definition));
        }
    }
    ...
    notify(name, value) {
        for (const listener of [...(this.subscribers.get(name) ?? [])])
            listener(value);
    }
...
function clampDefault(definition) {
