...
exports.selectAnimationTransition = selectAnimationTransition;
exports.transitionTriggerNames = transitionTriggerNames;
function selectAnimationTransition(machine, currentState, parameters, normalizedTime) {
...
function transitionTriggerNames(transition) {
    if (!transition.consumeTriggers)
        return [];
    return transition.conditions
        .filter((condition) => condition.operator === 'triggered')
        .map((condition) => condition.parameter);
}
function matchesConditions(transition, parameters) {
...
function matchesCondition(condition, parameters) {
