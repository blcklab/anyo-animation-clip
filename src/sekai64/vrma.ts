...
import { AnimationClip, type AnimationTrack as SekaiAnimationTrack } from '@blcklab/sekai64/animation';
...
import { prepareSource, object, correctGltfRestRotations, type AnimationImportContext, type Sekai64AnimationImporter, type TargetNode, type SecondaryNodePolicy } from './internal.js';
...
export function createVrmAnimationImporter(options: VrmAnimationImporterOptions = {}): Sekai64AnimationImporter {
  ...
  if (options.secondaryNodes !== undefined && !['portable', 'compatible', 'same-avatar', 'error'].includes(options.secondaryNodes)) throw new Error('Invalid VRMA secondaryNodes policy.');
  return {
    formats: ['vrma'],
    async load(context) {
...
function retarget(raw: GltfAnimationFinalizeContext, context: AnimationImportContext, bakeRate: number, unsupported: 'error' | 'skip', secondaryNodes: SecondaryNodePolicy): readonly AnimationClip[] {
  ...
  const exactTarget = (index: number): TargetNode | undefined => {
    if (rig.version !== '1' && secondaryNodes !== 'same-avatar') return undefined;
    ...
    if (secondaryNodes === 'same-avatar') { exactBindings.set(index, target); return target; }
    if (secondaryNodes === 'compatible' && !sameRest(sourceNode[0]!, target)) return undefined;
  ...
  try {
    for (const [clipIndex, animation] of (raw.document.animations ?? []).entries()) {
      context.signal.throwIfAborted();
      const rotations = new Map<Node, StepAnimationTrack>();
      const output: SekaiAnimationTrack[] = [];
      const seen = new Set<string>();
      let duration = 0;
      let exactPassThrough = 0;
      const skippedSecondary: string[] = [];
      const skipUnmapped = (message: string) => {
        if (secondaryNodes === 'error') throw new Error(message);
      ...
      for (const channel of animation.channels) {
        ...
        if (passthrough) {
          output.push(new CompatibleAnimationTrack({ target: passthrough.id, path: path as 'rotation' | 'translation' | 'scale', times, values, interpolation: sampler.interpolation === 'STEP' ? 'step' : 'linear' }));
          exactPassThrough++;
          continue;
        }
        const track = new StepAnimationTrack({ target: entry!.node.id, path: path as 'rotation' | 'translation', times, values, interpolation: sampler.interpolation === 'STEP' ? 'step' : 'linear' });
        ...
        if (entry!.kind === 'expression') {
          ...
          for (const expression of resolved) output.push(new CompatibleAnimationTrack({ target: expression.target, path: 'translation', times, values: weights, interpolation: track.interpolation }));
          continue;
        }
      ...
      if (exactPassThrough) context.onDiagnostic?.(`Preserved ${exactPassThrough} source-specific VRMA channel${exactPassThrough === 1 ? '' : 's'} using ${secondaryNodes} matching.`);
      if (skippedSecondary.length) {
        ...
        context.onDiagnostic?.(
          `Skipped ${skippedSecondary.length} source-specific VRMA channel${skippedSecondary.length === 1 ? '' : 's'} under ${secondaryNodes} matching` +
      ...
      for (const [name, source] of sourceBones) {
        ...
        const tracks = chain.map(node => rotations.get(node)).filter((track): track is StepAnimationTrack => Boolean(track));
