...
export function createSekai64AnimationClipLoader(options = {}) {
    return {
        async prepare(request) {
            const id = request.id.trim();
            if (!id)
                throw new Error('Prepared clip id is required.');
            ...
            if (!clips.length)
                throw new Error(`Animation asset "${id}" produced no clips.`);
            let disposed = false;
            return Object.freeze({ id, clips: Object.freeze(clips), target: request.target, restorePose: request.target.restorePose, dispose() { if (disposed)
                    return; disposed = true; for (const clip of clips)
                    clip.dispose(); } });
        },
    };
}
